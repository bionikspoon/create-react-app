...
const os = require('os');
const immer = require('@bionikspoon/react-dev-utils/immer').produce;
const globby = require('@bionikspoon/react-dev-utils/globby').sync;
    ...
    console.warn(
      chalk.yellow(
        `We detected TypeScript in your project (${chalk.bold(
          `client${path.sep}${typescriptFiles[0]}`
...
function verifyTypeScriptSetup() {
  ...
  if (parsedTsConfig.include == null) {
    appTsConfig.include = ['client'];
    messages.push(
      `${chalk.cyan('include')} should be ${chalk.cyan.bold('client')}`
    );
  }
